...
def suggest(temp, hum, rain):
    ...
    # Advanced crop yield prediction based on confidence score
    # Yield is estimated in quintals per acre (approximate calculation)
    base_yields = {
        "Rice": 20, "Wheat": 15, "Maize": 25, "Cotton": 10, "Sugarcane": 300,
        "Groundnut": 12, "Soybean": 10, "Tomato": 100, "Onion": 80, "Potato": 120,
        "Chilli": 40, "Turmeric": 60, "Ginger": 70, "Banana": 150, "Mango": 60,
        "Papaya": 180, "Coconut": 40, "Mustard": 8, "Sunflower": 10, "Brinjal": 90,
        "Okra": 50, "Cucumber": 100, "Watermelon": 150, "Spinach": 40, "Green Peas": 20,
        "Cauliflower": 80, "Cabbage": 90, "Carrot": 100, "Jowar": 12, "Bajra": 10, "Ragi": 15
    }
    
    base = base_yields.get(best["crop"], 30)
    # Scale yield by the suitability score (higher score = closer to maximum potential yield)
    estimated_yield = round(base * (best["score"] / 100) * 1.1, 1) # 10% bonus for optimal conditions
    ...
    return {
        ...
        "yield_note": "Yield prediction is based on the selected region's weather profile, assuming standard agricultural practices.",
