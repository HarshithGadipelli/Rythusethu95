...
export function parseSpokenNumber(text) {
  ...
  // Check if it has digits. If so, return all digits combined.
  if (/\d/.test(cleaned)) {
...
// Convert spoken sequences (phone numbers, pincodes, aadhaar) to string
...
export function playTTS(text, lang = "en", options = {}) {
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      if (!data.audioContent) return resolve(false);
      ...
      if (options.volume) audio.volume = Math.max(0.1, Math.min(1.0, options.volume));
      
      // Use Web Audio API to truly change the tone/timbre without affecting speed
      if (options.filterType) {
        try {
          ...
          filter.type = options.filterType; // 'highpass', 'lowpass', or 'bandpass'
          filter.frequency.value = options.filterFreq || 1000;
          // Add some resonance for the megaphone effect
          if (options.filterType === "bandpass") filter.Q.value = 2.5; 
          
          source.connect(filter);
          filter.connect(globalAudioCtx.destination);
          
          // Ensure context runs
          if (globalAudioCtx.state === "suspended") globalAudioCtx.resume();
        } catch(e) {
          console.warn("Web Audio API filter failed:", e);
        }
      }
      
      audio.onended = () => {
        currentAudio = null;
        if (currentTTSResolver) currentTTSResolver(true);
        resolve(true);
      };
